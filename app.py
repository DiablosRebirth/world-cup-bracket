...
def fetch_bracket_data():
    try:
        ...
        for group_data in data.get("standings", []):
            # Safely extract the group letter or identifier cleanly (handles text like 'Group K' smoothly)
        ...
        # Show the exact error on screen if the live link breaks instead of masking it
        st.sidebar.error(f"⚠️ API Fetch Error: {e}")

    # Keep your hardcoded backup map ONLY if the internet completely drops out
